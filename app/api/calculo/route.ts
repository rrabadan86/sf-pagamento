...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        // Coletar todos os IDs de membros ativos no mês
        ...
        // Ler enrollments do banco local (populado pelo cron noturno)
        ...
        // várias aparições no mês. Já uma REPOSIÇÃO é pontual: aparece 1 única vez.
        // Portanto: turma com >= LIMIAR_MATRICULA aparições = matrícula; 1 aparição = reposição.
        const LIMIAR_MATRICULA = 2;
        const recorrenteFixedTurmas = new Map<number, Set<string>>();
        {
            ...
            for (const [idMember, turmaCounts] of contagemPorMembro.entries()) {
                ...
                // Matriculada = turma com aparições recorrentes; reposição = 1 aparição pontual.
                const matriculadas = Array.from(turmaCounts.entries())
                    .filter(([, count]) => count >= LIMIAR_MATRICULA)
                    .map(([k]) => k);

                if (matriculadas.length > 0) {
                    recorrenteFixedTurmas.set(idMember, new Set(matriculadas));
                }
            }
            console.log(`[Cálculo] Turmas matriculadas inferidas para ${recorrenteFixedTurmas.size} membros FIXO sem grade.`);
        }
        ...
        for (const [pid, prof] of Object.entries(porProfessor)) {
            ...
            for (const [nomeTurma, aulas] of Object.entries(prof.turmas)) {
                ...
                for (const { aula, enrollments } of aulasComMatriculas) {
                    ...
                    // Injetar alunas fixas ausentes (feriado / zero enrollment)
                    ...
                    for (const [idMember, evoData] of Array.from(statusMatriculadas.entries())) {
                        ...
                        // Remover contratos "Circuito" em turmas SlimFit
                        if (!isCircuitoClass) {
            ...
            // Aplicar diárias compartilhadas de sábado
